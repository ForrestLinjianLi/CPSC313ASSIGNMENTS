...
static int cache_line_retrieve_data(cache_line_t *cache_line, size_t offset)
{
    ...
    int data = 0;
    for(int i = 0; i < 4; i++){
        data += cache_line->data[offset+i] << 8*i;
    }
    return data;
}
...
static cache_line_t *cache_set_find_matching_line(cache_t *cache, cache_set_t *cache_set, intptr_t tag)
{
    ...
    for (int i = 0; i < cache->associativity; ++i) {
        cur_line = cache_set->cache_lines[i];
        if (cache_line_check_validity_and_tag(cur_line, tag)) {
...
{
    /* TO BE COMPLETED BY THE STUDENT */
    intptr_t offset = (intptr_t)address & cache->block_offset_mask;
    ...
    intptr_t address_arg = (intptr_t)address;

    cache_set_t target_set = cache->sets[index];
    cache_line_t *target_line = cache_set_find_matching_line(cache, &target_set, tag);
    cache->access_count++;
    if (!target_line) {
        cache->miss_count++;
        target_line = cache_set_add(cache, &target_set, address_arg, tag);
        return cache_line_retrieve_data(target_line, offset);
    }
    return cache_line_retrieve_data(target_line, offset);
}
